...
public final class GetAverageGradeUseCase {
    ...
    public float getAverageGrade(String course) {
        ...
        String[] teamMem = team.getMembers();
        for(String member : teamMem) {
            Grade[] grade = gradeDataBase.getGrades(member);
            for(Grade g : grade) {
                sum += g.getGrade();
                count++;
            }
        ...
        for (int i = 0; i < team.getMembers().length; i++){
            Grade gradeObject = gradeDataBase.getGrade(team.getMembers()[i], course);
            float grade = gradeObject.getGrade();
            sum += grade;
